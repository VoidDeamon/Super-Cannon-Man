...
Scene* Engine::_menuScene = nullptr;
std::string Engine::_gameName;

static bool loading = false;
static bool loadingFromFile = false;
...
void Engine::Update() {
    ...
    pauseCooldown += dt;
    if (Keyboard::isKeyPressed(Keyboard::Escape) && pauseCooldown > 0.2f && _activeScene != _menuScene) {
        pause = !pause;
        pauseCooldown = 0.0f;
    }
...
void Engine::setMenuScene(Scene* s) {
    _menuScene = s;
}
...
void Engine::SetPauseState(bool pauseState) {
    pause = pauseState;
}

bool Engine::getLoadingFromFile() {
    return loadingFromFile;
}

void Engine::setLoadingFromFile(bool lff) {
    loadingFromFile = lff;
}
